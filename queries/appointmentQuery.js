...
const createAppointmentQuery = async (time, patient, doctor) => {
    const result = await client.query(`
        INSERT INTO appointment (time, patient, doctor)
        VALUES ($1, $2, $3) RETURNING *;
    `, [time, patient, doctor]);

    const aid = result.rows[0]._id; 

    await client.query(`INSERT INTO ptakes (aid, pid) VALUES ($1, $2);`, [aid, patient]);
