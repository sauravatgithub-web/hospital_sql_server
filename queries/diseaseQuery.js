...
const createDiseaseQuery = async (name, symp, description) => {
    return await client.query(
        'INSERT INTO Disease (name, symp, description, active) VALUES ($1, $2, $3, TRUE) RETURNING *;',
        [name, symp,description]
    );
};

const updateDiseaseQuery = async (id, name, symp, description) => {
    return await client.query(
        'UPDATE Disease SET name = $1, symp = $2, description = $3 WHERE _id = $4 RETURNING *;',
        [name, symp, description , id]
    );
};
